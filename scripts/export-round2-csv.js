/*
  Export Round 2 scenario keys to CSV.
  Columns: Sector, Subsector, Round 1 Measure, Round 2 Measure, coins, score, cutscene
*/
...
function extractRoundTwoEntries(scenarioTs, activityMap, cutsceneMap) {
  const objRegex = /export const roundTwoScenarioConfiguration:[\s\S]*?=\s*\{([\s\S]*?)\n\};/;
...
function parseEntryToRow(entry) {
  const key = entry.key;
  // Format: `${sector}_${subSector}_${round1Measure}-${round2Measure}-${meanSeaLevel}-${randomizeEffect}`
  const m = key.match(/^(.+?)-([^-]+)-([^-]+)-([^-]+)$/);
  if (!m) {
    return { sector: '', subsector: '', round1: key, round2: '', coins: entry.coin, score: entry.score, cutscene: entry.cutscene };
  }
  const [, prefix, round2Measure, meanSeaLevel, randomizeEffect] = m; // eslint-disable-line @typescript-eslint/no-unused-vars
  const [sector, subSector, ...round1Parts] = prefix.split('_');
  const round1Measure = round1Parts.join('_');
  return { sector, subsector: subSector, round1: round1Measure, round2: round2Measure, coins: entry.coin, score: entry.score, cutscene: entry.cutscene };
}

function toCsv(rows) {
  const headers = ['Sector', 'Subsector', 'Round 1 Measure', 'Round 2 Measure', 'coins', 'score', 'cutscene'];
...
function main() {
  ...
  const entries = extractRoundTwoEntries(scenarioTs, activityMap, cutsceneMap);
  const rows = entries.map(parseEntryToRow);
  const csv = toCsv(rows);

  const outDir = path.join(repoRoot, 'exports');
  fs.mkdirSync(outDir, { recursive: true });
  const outPath = path.join(outDir, 'round-two-keys.csv');
