/*
  Export Optional scenario keys to CSV.
...
function extractOptionalEntries(scenarioTs, activityMap, cutsceneMap) {
  // Allow optional semicolon after the closing brace, or none
  const objRegex = /export const optionalScenarioConfiguration:[\s\S]*?=\s*\{([\s\S]*?)\n\}\s*;?/;
...
function main() {
  ...
  const entries = extractOptionalEntries(scenarioTs, activityMap, cutsceneMap);
  const rows = entries.map(parseEntryToRow);
  const csv = toCsv(rows);

  const outDir = path.join(repoRoot, 'exports');
  fs.mkdirSync(outDir, { recursive: true });
  const outPath = path.join(outDir, 'optional-keys.csv');
